...
fork하고 `config.yml`·`members.yml`을 자기 보드에 맞게 고친 뒤 자기 Notion 토큰을
시크릿에 넣으면 된다. 토큰 보관 책임이 각자에게 남으므로 OAuth 서버도 DB도 필요
없다.

보드 스키마가 조직마다 다른 건 `config.yml` 하나로 흡수된다. 진짜 병목은 인증이고,
그건 fork-and-configure 모델이 원천적으로 피해 간다.
